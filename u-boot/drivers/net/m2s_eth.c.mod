...
static  int phy_get_link_status(MAC_speed_t * speed, uint8_t * fullduplex);
#endif
...
static int msgmii_phy_init(void)
{
  ...
  printf("msgmii_phy_init. address = 0x%x \n", M88E1340_PHY_ADDR);  
  
  /// from MSS_MAC in SoftConsole ////
  /* Reset the PHY. */
  printf("Reset the PHY\n");
  rv = miiphy_read(M2S_MII_NAME, M88E1340_PHY_ADDR, PHY_BMCR, &val);  
  if (rv != 0) {
    return -1;
  }
  val |= PHY_BMCR_RESET;
...
static int m881340_phy_set_link_speed(void)
{
  ...
  int rv;
  uint32_t inc;
  uint32_t speed_duplex_select = MSS_MAC_ANEG_ALL_SPEEDS;
  ...
  printf("Set 10Mbps and 100Mbps advertisement.\n");  
  ...
  printf("Set 1000Mbps advertisement.\n");
  val = 0;
...
static int msgmii_phy_autonegotiate(void)
{
  ...
  */
  ...
  printf("Wait for copper auto-negotiation to complete.\n");
  val = 0;
  timeout = copper_aneg_timeout ; //M2S_AUTONEG_TOUT/1000;
  do {
    rv = miiphy_read(M2S_MII_NAME, M88E1340_PHY_ADDR,
		     PHY_BMSR, &val);
    autoneg_complete = val & PHY_BMSR_AUTN_COMP;
    --timeout;
  } while(!autoneg_complete && (timeout != 0u));
  ...
  printf("phy_get_link_status. %d - 0x%x\n", copper_link_up, link_speed);  
  val = 0;
  ...
  if(1u == copper_link_up){        
    ...
    M2S_SYSREG->mac_cr = (M2S_SYSREG->mac_cr &  ~MAC_CONFIG_SPEED_MASK) | link_speed;
    ...
    rv = miiphy_read(M2S_MII_NAME, SF2_MSGMII_PHY_ADDR, 0x00, &val);
    if (rv != 0) {
      goto out;
    }

    val |= 0x1000;
    rv = miiphy_write(M2S_MII_NAME, SF2_MSGMII_PHY_ADDR, 0x00, val);
    if (rv != 0) {
      goto out;
    }
    val |= 0x200;
    ...
    printf("Wait until auto-negotiation completes...\n");
    timeout = sgmii_aneg_timeout; //M2S_AUTONEG_TOUT/10;    
    val = 0;
    do {
  ...
  */
  ...
  while (timeout--) {
	...
	continue;
      ...
      // restart auto-negotiation if it is not complete in a second 
...
static int phy_get_link_status( MAC_speed_t * speed,    uint8_t *     fullduplex ){
  ...
  copper_link_up = val & PHY_BMSR_LS;

  printf("phy_get_link_status. copper_link_up = 0x%x\n", copper_link_up);
  ...
  if(copper_link_up != 0u){
    ...
    printf("phy_get_link_status. M88E1340_PHY_STATUS = 0x%x\n", val);

    duplex = val & M88E1340_PHY_STATUS_FULLDUPLEX;
    phy_speed = val & M88E1340_PHY_STATUS_SPD_MASK;
