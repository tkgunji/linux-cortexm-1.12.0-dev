...
//#define STM_ID_M25P16		0x05
//#define STM_ID_M25P16		0x0b
...
static int stmicro_wait_ready(struct spi_flash *flash, unsigned long timeout)
{
  ...
  do {
    ...
    if ((status & STMICRO_SR_WIP) == 0){
      printf(" stmicro_wait_ready v2:: status register = 0x%x\n", status);
      break;
    }
  ...
  */
  ...
  do {
    ...
    if ((status & STMICRO_SR_WIP) == 0){
      printf(" stmicro_wait_ready:: status register = 0x%x\n", status);
      break;
    }
  } while (get_timer(timebase) < timeout);
			 ...
			 u32 offset, size_t len, const void *buf)
{
  ...
  for (actual = 0; actual < len; actual += chunk_len) {
    ...
    //ret = spi_flash_cmd(flash->spi, CMD_M25PXX_RDSR, &status, sizeof(status));
...
int stmicro_erase(struct spi_flash *flash, u32 offset, size_t len)
{
  ...
  if (offset % sector_size || len % sector_size) {
    ...
    printf("SF: Erase offset/length not multiple of sector size 0x%x\n", sector_size);
    return -1;
  }
