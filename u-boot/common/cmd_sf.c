...
static int do_spi_flash_read_write(int argc, char *argv[])
{
  ...
  printf("map_physmem: addr=0x%x, length=0x%x\n",
	 addr, len);
  ...
  printf("map_physmem OK and read/write cmd!!\n");
  ...
  printf("read/write cmd execution is OK. next is unmap physmem!!\n");

  unmap_physmem(buf, len);
  printf("unmap physmem OK!!\n");
  if (ret) {
